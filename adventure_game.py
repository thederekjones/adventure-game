...
def intro(ancient_artifact, enemy):
    print_pause(
        "You awaken in a dark room with an old," " dirty lamp lit in the corner."
    )
    print_pause(
        "How did you end up here? The last thing" " you remember is being in your bed."
    )
    print_pause(
        "You slowly stand up and go grab the lamp" " so you can get a good look around."
    )
    print_pause(
        "A desk is situated on the side of the room"
        " with a clean, white piece of paper."
    )
    print_pause(
        "There is a message written for you. A" f" warning? **Beware of the {enemy}!**"
    )
    print_pause(
        "You want to escape. To your left is a door," " and to your right is a window."
    )
...
def door(ancient_artifact, enemy):
    if "blade" in ancient_artifact:
        print_pause(
            "You slowly open the door. The trunk" " is still open with nothing inside."
        )
        print_pause("You go back into the old bedroom and" " shut the door behind you.")
    else:
        print_pause(
            "You open the door. It appears to be a"
            " large closet. Inside is a wooden trunk."
        )
        print_pause(
            "It doesn't seem to be locked. You open"
            " the trunk and find a note and a blade."
        )
        print_pause(f"**Behold! THE {enemy.upper()} BLADE!**")
        print_pause(
            "You grab your new weapon and go back to"
...
def window(ancient_artifact, enemy):
    print_pause("You go to the window. You're in luck! The" " window is unlocked.")
    print_pause("You slide it open and pull yourself through" " it carefully.")
    print_pause("You get safely outside when you hear rustling" " in the trees.")
    print_pause("Something is coming out of the woods. Oh no!" f" It's the {enemy}!")

    if "blade" not in ancient_artifact:
        print_pause(f"The {enemy} is coming toward you and you" " don't have a weapon!")
        print_pause("You fight bravely, but you were no match" " empty handed.")
        print_pause("GAME OVER!")
        new_game()
    else:
        print_pause(f"The {enemy} is coming toward you. You're" " ready for a fight!")
        print_pause("You swing wildly, eventually connecting" f" with the {enemy}.")
        print_pause(
            "It yells out in pain. It falls to the ground" " and turns to dust."
        )
        print_pause(
            "You've won! A light shines in the distance,"
            " getting brighter and brighter."
        )
        print_pause(
            "Eventually all you can see is a bright, white"
...
def new_game():
    ...
    response = input(
        "Would you like to play again? Please" " enter 'yes' or 'no' now.\n"
    ).lower()

    if response == "yes":
        print_pause(
            "You've been sprinkled with sleeping dust." " You enter a deep sleep."
        )
        start_game()
    elif response == "no":
        print_pause(
            "You walk into the kitchen and make some"
...
def start_game():
    ancient_artifact = []
    enemy = random.choice(
        ["werewolf", "vampire", "skinwalker", "ghoul", "shapeshifter"]
    )
